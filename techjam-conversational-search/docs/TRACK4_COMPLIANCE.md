...
The deterministic v2.15.0 run on all 200 released development sessions produces:

- Hit Rate@10: `1.0` (200/200)
- MRR: `0.996`
- MTTC: `2.055`
- Efficiency: `0.8945`
- Technical Score: `0.9777`
- Reported model tokens and API cost: `0`

Buying, Browsing, boundary, and intent-override Hit Rate@10 are all `1.0`. The v2.13.0
miss was a retrieval-recall limit — the target never entered the candidate pool — and is
resolved by the conjunctive category-recall route, not by a scoring change.
...
harness from catalog targets the public set never uses.

Two held-out sets are reported because v2.15.0's popularity prior is sensitive to how
targets are sampled. Public targets have a median `rating_number` of ~7,000 against a
catalog median of 12, since a session requires an anonymized preference profile and that
requires reviews.
...
The unreleased sessions are produced by the same pipeline as the public ones, so the
matched row is the representative estimate. `POPULARITY_PRIOR_WEIGHT = 0.0` disables the
prior and restores v2.14.0 ranking behavior if that assumption does not hold.
