...
> constraint card as one blob. A joined string can never equal a single card entry, so the
> exact-match branch silently degraded to the 0.25x partial-coverage branch and the
> strongest ranking signal was lost precisely when the shopper had given the most
> information. Adds an **open-ended-first clarification policy** (an unscoped question
> collects whatever the shopper considers important; targeted attribute questions take
> over once it stops yielding anything new), holds the one-item precision slate through
> turn 5 on every route, lowers `EVIDENCE_BOOST_WEIGHT` `6 -> 2` (it was double-counting
> with the now-correctly-matching card), and adds a shallow leading-position card decay as
> a tie-breaker. Public set: MRR `0.948458 -> 0.976806`, MTTC `3.315 -> 2.81`, Efficiency
> `0.7685 -> 0.819`, Technical Score **`0.938237 -> 0.954342`**, Hit Rate@10 `1.0 -> 0.995`,
> zero model tokens.
>
> Validated on a 400-session held-out set generated by the same harness from catalog
> targets the public set never uses, where the same change set moves the score
> `0.910664 -> 0.933532` — a larger gain than on the public set, so the improvement is not
> public-set overfitting.
>
...
- **Ask attribute selection** (`_choose_ask_attribute`) is two-stage as of v2.13.0:
  1. **Open-ended first.** While the shopper is still volunteering requirements, ask an
     unscoped question (`other`). A question scoped to one attribute can only ever return
     that attribute; an unscoped one returns whatever the shopper actually considers
     important, so it collects strictly more per turn.
  2. **Targeted once it dries up.** After `OPEN_ASK_DRY_LIMIT` consecutive open questions
     return no new constraint, fall back to the original entropy rule: compute the
     discriminating power of each attribute's value distribution over the candidate pool
     and ask about the attribute that best splits it, skipping attributes already asked.

  The limit is `2` rather than `1` because a shopper legitimately answers the very first
  question with "no preference, use your judgment" while still having constraints left to
  give — giving up after a single dry answer strands those sessions.
...
| `FUSION_WEIGHTS` | `{bm25:2.5, dense:1.8, slot:3.2, price:0.0, bias:-3.5}` | five-fold mean weights fitted on the public development set |
